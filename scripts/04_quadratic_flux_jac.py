...
from scripts.universal import init_modular, init_saddle
...
x = objective.x()
print(
    f"\nQuadraticFlux.jac_scaled_error for {name}, {n_modular} modular + "
    f"{n_saddle} saddle coils ({field.num_coils} total incl. virtual coils)"
)
...
def run():
    return objective.jac_scaled_error(x).block_until_ready()
